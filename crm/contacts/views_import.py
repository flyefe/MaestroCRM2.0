        ...
        for row in csv_data:
            ...
            for csv_column, model_field in header_mapping.items():
                ...
                if model_field == 'first_name':
                    ...
                    try:
                        # Use the utility function to handle multiple date formats
                        contact_data[model_field] = parse_date(value) if value else None
                    except ValueError:
                        # Log an error if all formats fail
                        messages.warning(request, f"Invalid date format for '{csv_column}' in value '{value}'. Row skipped.")
                        continue
